...
// ===== EXACT BOOT LOGO FROM VIDEO =====
void showBootLogo() {
  oled.setTextColor(1);
  
  // Frame 1: Tiny center pulse
  oled.clearDisplay();
  sFillRect(62, 30, 4, 4);
  oled.display();
  safeDelay(200);
  
  // Frame 2: [ NIVI ] and LABS
  oled.clearDisplay();
  oled.setTextSize(2);
...
void handleRoot() {
  String h;
  h.reserve(1024); // Prevents heap fragmentation crash
...
void processAI() {
  ...
  if(millis()-lastAITick>5000) {
    ...
    if(isNight&&energy<50) mood=1; else if(memoryMood<20) mood=2;
    else if(energy<15) mood=6; else if(boredom>70) mood=4;
...
void drawFace(int m) {
  ...
  float lDrawW=le.w+(30-le.h)*0.2, rDrawW=re.w+(30-re.h)*0.2;
  le.update(); re.update();
...
void anim2_ExcitedPulse() { oled.clearDisplay(); unsigned long t = millis() - animStart; float pulse = sin((float)t / 400.0f) * 0.5f + 0.5f; int size = (int)(10 + pulse * 12), lx = 38, rx = 90; sFillCircle(lx, 32, size, 1); sFillCircle(rx, 32, size, 1); sFillCircle(lx, 32, size/3, 0); sFillCircle(rx, 32, size/3, 0); oled.display(); }
void anim3_ConfidentStare() { oled.clearDisplay(); unsigned long t = millis() - animStart; int lx = 38, rx = 90; sFillRoundRect(lx-14, 28, 28, 8, 4); sFillRoundRect(rx-14, 28, 28, 8, 4); float shimmer = sin((float)t / 300.0f); if(shimmer > 0) { sFillRect(lx-10, 28, 20, 8, 0); sFillRect(rx-10, 28, 20, 8, 0); sHLine(lx-8, 28, 16); sHLine(rx-8, 28, 16); } oled.display(); }
...
void anim12_SelfPlayingGame() {
  oled.clearDisplay(); unsigned long gameTime = (millis() - animStart) % 8000; int lx = 38, rx = 90, ly = 12;
...
void runAnim() {
  ...
  switch(curAnim) {
    case 1: anim1_HappyBounce(); break; case 2: anim2_ExcitedPulse(); break; case 3: anim3_ConfidentStare(); break;
    case 4: anim4_PlayfulWink(); break; case 5: anim5_CuriousGaze(); break;  case 6: anim6_DreamyFloat(); break;
    case 7: anim7_DeterminedFocus(); break; case 8: anim8_SurprisedGasp(); break; case 9: anim9_LovingGlow(); break;
    case 10: anim10_NervousFlutter(); break; case 11: anim11_ProudDisplay(); break; case 12: anim12_SelfPlayingGame(); break;
    default: drawFace(mood); break;
    // ===== ARCADE GAMES (FIXED NON-BLOCKING TOUCH) =====
void playDino() {
  ...
  while(1) {
    ...
    if(isTouched && !wasTouched) {
      ...
      lastTap=millis(); lastInteraction=millis();
      if(y>=40) dy=-8.5;
    }
...
void setup() {
  ...
  hiScoreDino = prefs.getInt("hdino", 0); hiScoreSnake = prefs.getInt("hsnake", 0);
  hiScoreFlappy = prefs.getInt("hflappy", 0); memoryMood = prefs.getInt("memMood", 50);
  prefs.end();
  
  if(ssid=="") {
    curState=CONFIG; WiFi.softAP("Nivi-Setup"); dnsServer.start(53,"*",WiFi.softAPIP());
  } else {
...
void loop() {
  ...
  if(ssid!=""&&WiFi.status()!=WL_CONNECTED) {
    if(millis()-lastWifiRetry>45000) { WiFi.reconnect(); lastWifiRetry=millis(); }
  }
  
  if(curState!=CONFIG&&WiFi.status()==WL_CONNECTED&&millis()-lastWeather>300000) { fetchWeather(); lastWeather=millis(); }
  
  static unsigned long lastSave=0;
  if(millis()-lastSave>180000) { prefs.begin("nivi",false);prefs.putInt("memMood",memoryMood);prefs.end(); lastSave=millis(); }
  ...
  if(touchEnabled&&digitalRead(TOUCH_PIN)) {
    ...
    while(digitalRead(TOUCH_PIN)) {
      ...
      if(curState==FACES) {
        ...
        if(millis()-st>800&&!longPetTriggered) {
          ...
          affection=constrain(affection+15,0,100); boredom=0; energy=constrain(energy+10,0,100);
          memoryMood=constrain(memoryMood+5,10,90); mood=3;
    ...
    if(dur>600) { clickType=3; } 
    else {
      unsigned long wt=millis(); bool dbl=false;
      while(millis()-wt<250){
        if(digitalRead(TOUCH_PIN)){delay(10);if(digitalRead(TOUCH_PIN)) {dbl=true;while(digitalRead(TOUCH_PIN))yield();break;}} yield();
      }
      clickType=dbl?2:1;
    }
    ...
    if(curState==FACES) {
      if(clickType==2) curState=MAIN_MENU;
        ...
        animReacted=true; animReactTime=millis(); rapidTaps++;
      ...
      if(clickType==1) menuIdx=(menuIdx+1)%6;
        ...
        if(menuIdx==0) curState=FACES; else if(menuIdx==1) curState=CLOCK;
        else if(menuIdx==2){curState=WEATHER;if(!weatherLoaded)fetchWeather();} 
        else if(menuIdx==3) curState=ARCADE_MENU; else if(menuIdx==4){curState=FOCUS;focusStart=millis();}
  ...
  switch(curState) {
    ...
    case MAIN_MENU: {
      oled.clearDisplay(); const char* m[]={"Bio-Face","Time","Weather","Arcade","Focus","Reset"};
      for(int i=0;i<6;i++){
        int y=6+i*10;
    ...
    case ARCADE_MENU: {
      oled.clearDisplay(); const char* g[]={"Dino","Snake","Flappy","Back"};
      for(int i=0;i<4;i++){
        int y=12+i*13;
    ...
    case CLOCK: {
      struct tm ti; getLocalTime(&ti); oled.clearDisplay();
      int hr=ti.tm_hour%12; if(hr==0) hr=12;
      oled.setTextSize(2); oled.setCursor(8,20); oled.printf("%02d:%02d:%02d",hr,ti.tm_min,ti.tm_sec);
      oled.setTextSize(1); char ds[20]; strftime(ds,sizeof(ds),"%d-%b-%Y",&ti);
      oled.setCursor(31,42);oled.print(ds); oled.display(); break;
    }
    case WEATHER: {
      oled.clearDisplay(); oled.setTextSize(3); oled.setCursor(10,20); oled.print(tempC);
      oled.drawCircle(48,22,4,1); oled.drawCircle(48,22,3,1); oled.drawFastVLine(64,15,34,1);
      ...
      if(w.indexOf("cloud")>=0){
        ...
        oled.drawCircle(95,20,6,1); int dy2=30+(millis()%1000)/100;
        sDrawLine(90,constrain(dy2,0,60),88,constrain(dy2+3,0,63)); sDrawLine(100,constrain(dy2,0,60),98,constrain(dy2+3,0,63));
      } else {
        sFillCircle(95,24,6);
        if(millis()%1000>500) {sHLine(85,24,4);sHLine(103,24,4);sVLine(95,14,4);sVLine(95,32,4);}
      }
      oled.setTextSize(1); oled.setCursor(68,40);
      oled.print(weatherDesc.substring(0,min((unsigned int)weatherDesc.length(),(unsigned int)9))); oled.display(); break;
    }
    case FOCUS: {
      ...
      if(rem<=0){
        ...
        le.ty=32;re.ty=32;le.th=24;re.th=24; reactionTimer=millis()+3000; curState=FACES; break;
      }
      oled.clearDisplay(); oled.setTextSize(2); oled.setCursor(30,20);
      oled.printf("%02d:%02d",(int)(rem/60),(int)(rem%60)); oled.drawRoundRect(14,45,100,8,3,1);
      int pw=constrain((int)map(elapsed,0,25*60,0,96),0,96); sFillRoundRect(16,47,pw,4,2);
    ...
    case NOTIFY: {
      if(notifyStart==0) notifyStart=millis(); oled.clearDisplay();oled.drawRect(0,0,128,64,1);
      oled.setCursor(5,10);oled.print(remoteMsg);oled.display();
      if(millis()-notifyStart>4000){notifyStart=0;curState=FACES;} break;
    }
